...
const URL = `${SITE}/gestion-locative-casablanca`;

const PILLARS = [
  {
    icon: TrendingUp,
    title: "Revenus optimisés",
    desc: "Pricing dynamique calé sur la demande business casablancaise, optimisation des annonces, multi-canaux. On vise le rendement maximum toute l'année.",
  },
  {
    icon: Eye,
  ...
  },
  {
    ...
    desc: "Un partenaire Daribnb dédié à Casablanca, disponible 7j/7 pour l'accueil voyageurs, le ménage et la maintenance. Pas de gestion à distance approximative.",
  },
  {
    icon: Wallet,
...
const STEPS = [
  {
    ...
    desc: "On audite votre bien, on étudie le marché Casablanca sur votre quartier, on vous donne une fourchette de revenus réaliste sous 24h.",
  },
  {
    num: "02",
...
export default function GestionLocativeCasablanca() {
  const schema = {
    "@context": "https://schema.org",
    "@type": "Service",
    name: "Gestion locative Casablanca — Location courte durée Airbnb",
    ...
    description:
      "Service complet de gestion locative pour appartements et maisons à Casablanca. Location courte durée Airbnb, équipe locale, transparence et revenus optimisés.",
    url: URL,
  };

  return (
    <ServicePageLayout
      title="Gestion locative à Casablanca | Daribnb — Location courte durée Airbnb"
      description="Confiez la gestion locative de votre appartement à Casablanca à Daribnb. Équipe locale, transparence totale, revenus optimisés. Estimation gratuite en 24h."
      canonical={URL}
      h1="Gestion locative à Casablanca"
      kicker="Location courte durée · Casablanca"
      intro="Daribnb gère votre bien à Casablanca en location meublée courte durée (Airbnb, Booking, Vrbo), avec une équipe locale sur place. On s'occupe de tout : annonces, voyageurs, ménage, maintenance. Vous récupérez votre revenu, sans la charge mentale."
      schema={schema}
    >
      {/* 4 piliers */}
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-5xl mx-auto px-6 md:px-12">
          <div className="max-w-3xl mb-8">
            ...
            <h2 className="mt-3 text-3xl md:text-4xl font-bold text-[#1A1A1A]">
              Casablanca, quartier par quartier.
            </h2>
            <p className="mt-4 text-lg text-[#4B5563]">
              Notre équipe locale gère des biens dans les quartiers stratégiques de Casablanca. La rentabilité varie fortement selon l'emplacement — on connaît les chiffres réels par zone.
            </p>
          </div>

          <div className="grid md:grid-cols-3 gap-4">
            {[
              { name: "Maarif", note: "Clientèle business, forte demande semaine" },
              { name: "Gauthier", note: "Standing, proche centre d'affaires" },
              { name: "Racine", note: "Résidentiel premium, familles diaspora" },
              { name: "Anfa", note: "Haut de gamme, vue et calme" },
              { name: "Ain Diab", note: "Bord de mer, demande loisirs été" },
              { name: "Habous / Corniche", note: "Touristes internationaux" },
            ].map((q) => (
              <div
                key={q.name}
